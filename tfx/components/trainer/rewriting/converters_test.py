...
class RewritingExporterTest(tf.test.TestCase):
  ...
  class TestRewriter(rewriter.BaseRewriter):

    def __init__(self, rewrite_result):
      """Initializes the MyRewriter class.

      Args:
        rewrite_result: The value to return upon invocation of a rewrite
          operation.
      """
  ...
  def testRewritingExporterSucceeds(self, base_exporter_mock):
    ...
    base_exporter_mock.side_effect = export_fn

    tr = self.TestRewriter(True)
    ...
    print('Expect path:' + os.path.join(
        final_path, tf.saved_model.ASSETS_DIRECTORY, REWRITTEN_VOCAB))
    self.assertTrue(

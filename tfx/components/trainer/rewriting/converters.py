...
"""Converters rewrite the provided models using the provided rewriters."""
...
def _invoke_rewriter(src, dst, rewriter_inst, src_model_type, dst_model_type):
  ...
  Raises:
    RuntimeError: if the model is unable to perform a rewrite or if the
      source path is the same as the destination path.
  """

  if src == dst:
    raise RuntimeError('Source path and destination path cannot match.')

  original_model = rewriter.ModelDescription(src_model_type, src)
  rewritten_model = rewriter.ModelDescription(dst_model_type, dst)

  if not rewriter.perform_rewrite(original_model, rewritten_model,
                                  rewriter_inst):
    raise RuntimeError('Unable to perform rewrite')


class RewritingExporter(tf.estimator.Exporter):
  """This class invokes the base exporter and a series of rewriters."""

  def __init__(self, base_exporter, rewriter_inst):
  ...
  def export(self, estimator, export_path, checkpoint_path, eval_result,
    ...
    Raises:
      RuntimeError: Unable to perform the rewrite.
    """
    ...
    _invoke_rewriter(base_path, tmp_rewrite_path,
                     self._rewriter_inst,
...
def rewrite_saved_model(src, dst, rewriter_inst,
                        src_model_type=rewriter.ModelType.SAVED_MODEL,
  ...
  Args:
    ...
    dst_model_type: the `rewriter.ModelType` of the destination model.
  """
  _invoke_rewriter(src, dst, rewriter_inst, src_model_type,
                   dst_model_type)
